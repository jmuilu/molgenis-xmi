...
class XMIVersionHandler(val XMI: Ns, val UML: Ns, val version : String = "") extends LazyLogging {
  ...
  def parseNodeType(currentNode: XNode): Option[String] = {
   if ( !currentNode.n.attribute(XMI, "type").isDefined) {
      assert ( currentNode.n.label == "Model")
      return Some("uml:Model" ) 
   } else {
	   toOptStr(currentNode.n.attribute(XMI, "type")) 
   }
  }
...
object XMIVersionHandler {
  ...
  val EA_XMI21 = new XMIVersionHandler(Ns("xmi", "http://schema.omg.org/spec/XMI/2.1"), Ns("uml", "http://schema.omg.org/spec/UML/2.1"),"2.1") // not supported 
